...
const SECTIONS = [
  ...
  ['Capitale a scadenza',
    'Per ogni scadenza (un anno, un semestre o una data precisa) l\'app guarda i titoli del paniere che scadono in quella finestra e sceglie la combinazione con il rendimento netto più alto, rispettando la quota massima per emittente (con un algoritmo esatto: non lascia mai scoperta una scadenza che si può coprire). Poi calcola quanto comprare partendo dall\'ultima scadenza: ogni importo è il rimborso del suo titolo più le cedole incassate da tutti i titoli nel suo periodo, cioè il suo anno o semestre o i 12 mesi prima della data (se l\'opzione è attiva), arrotondato al lotto minimo. Le cedole incassate prima della scala, o fra due date lontane, non contano: resterebbero ferme per anni e, in una scala che parte fra molto tempo, prenderebbero il posto del primo titolo. Restano a te: le spendi o le reinvesti. Con le date precise conta il "rendimento effettivo alla data": se il titolo scade prima, i soldi restano fermi fino a quel giorno.'],
  ['Rendita mensile',
